...
def launch_lerobot_train(
    ...
    cmd = [
        sys.executable, "-c",
        "import lerobot.datasets.utils as u, lerobot.datasets.video_utils as vu, lerobot.datasets.dataset_metadata as dm, lerobot.datasets.storage as st, lerobot.datasets.io_utils as io, lerobot.datasets.dataset_reader as dr, datasets, torch, numpy as np, av; u.check_version_compatibility = lambda *a, **k: None; _orig_ft = dm.LeRobotDatasetMetadata.features.fget; dm.LeRobotDatasetMetadata.features = property(lambda self: {k: v for k, v in _orig_ft(self).items() if 'depth' not in k}); _orig_fp = datasets.Dataset.from_parquet; datasets.Dataset.from_parquet = lambda paths, *a, **k: _orig_fp(paths, *a, **{**k, 'features': None}); _orig_dec = vu.decode_video_frames_pyav; vu.decode_video_frames_pyav = lambda vp, ts, tol, log_loaded_timestamps=False, return_uint8=False, is_depth=False: (torch.stack([(torch.from_numpy(f.to_ndarray(format='gray16le').astype(np.int32))) for f in av.open(str(vp)).decode(av.open(str(vp)).streams.video[0])][:len(ts)]) if is_depth else _orig_dec(vp, ts, tol, log_loaded_timestamps=log_loaded_timestamps, return_uint8=return_uint8, is_depth=is_depth)); _orig_ds = dr.DatasetReader._decode_single; dr.DatasetReader._decode_single = lambda self, vid_key, ep_idx, ts: (_orig_ds(self, vid_key, ep_idx, ts) if vid_key in self._depth_encoder_configs or vid_key not in self._meta.depth_keys else None); from lerobot.scripts.lerobot_train import main; main()",
